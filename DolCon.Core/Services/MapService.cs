...
public class MapService : IMapService
{
    ...
    public MapService(IPlayerService playerService, IPositionUpdateHandler positionUpdateHandler)
    {
        ...
        InstallShippedWorlds(_worldsPath);
    }

    /// <summary>
    /// Copies the baked worlds that ship with the game into the user's Worlds folder, refreshing any
    /// that are out of date. Baked worlds are build artifacts regenerated by WorldForge, so a stale
    /// installed copy would silently keep the player on an old world; overwriting is safe because
    /// the canonical world is immutable and progress-free (progress lives in the save game).
    /// </summary>
    private static void InstallShippedWorlds(string worldsPath)
    {
        // BaseDirectory is where the game actually lives; CurrentDirectory is a fallback for
        // `dotnet run`, which sets the working directory to the project folder.
        foreach (var shipped in new[] { AppContext.BaseDirectory, Environment.CurrentDirectory })
        {
            var shippedWorlds = Path.Combine(shipped, "Worlds");
            if (!Directory.Exists(shippedWorlds)) continue;

            foreach (var world in Directory.GetFiles(shippedWorlds, $"*{WorldFileExtension}"))
            {
                var destination = Path.Combine(worldsPath, Path.GetFileName(world));
                if (File.Exists(destination) &&
                    File.GetLastWriteTimeUtc(destination) >= File.GetLastWriteTimeUtc(world))
                {
                    continue;
                }

                File.Copy(world, destination, overwrite: true);
            }
